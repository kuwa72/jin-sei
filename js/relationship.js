...
class Relationship {
    ...
    updateStrength(deltaTime) {
        ...
        // 基本的な強化率
        let strengthIncrease = 0.001 * deltaTime;
        
        // 性格特性による影響
        const compatibility = Genetics.calculateCompatibility(this.entity1, this.entity2);
    ...
    checkDissolve() {
        ...
        // 関係の強さに基づく解消確率
        // const dissolveChance = 0.0001 * (1 - this.strength);
        // const dissolveChance = 0.0001 * (1 - Math.pow(this.strength, 2));
        //const dissolveChance = 0.0001 * (1 - Math.pow(this.strength, 3));
        const dissolveChance = 0.001 * (1 - this.strength);
    ...
    dissolve() {
        if (this.type === 'partner') {
            ...
            // 関係リストから削除
            this.entity1.relationships = this.entity1.relationships.filter(r => r.entity !== this.entity2);
            this.entity2.relationships = this.entity2.relationships.filter(r => r.entity !== this.entity1);
        }
    }
}
...
class RelationshipSystem {
    ...
    createRomanticRelationship(entity1, entity2) {
        ...
        const existingRelationship = this.romanticRelationships.find(
            r => (r.entity1 === entity1 && r.entity2 === entity2) || 
    ...
    updateRelationships(deltaTime) {
        ...
        for (let i = this.romanticRelationships.length - 1; i >= 0; i--) {
            ...
            if (this.simulation.time - romance.startTime > romance.duration) {
                ...
                this.simulation.logger.logRomanceFailed(romance.entity1, romance.entity2);
            }
        }
    ...
    checkNewRelationships() {
        ...
        const adultEntities = entities.filter(e => !e.isDead && e.lifeStage === 1 && !e.partner);
    ...
    findPotentialPartner(entity) {
        ...
        for (const other of entities) {
            ...
            if (other === entity || other.isDead || other.lifeStage !== 1 || 
                other.gender === entity.gender || other.partner) {
                continue;
            }
            ...
            if (distance < 50) { // 近くにいる場合
                ...
                // 相性をチェック
                const compatibilityScore = Genetics.calculateCompatibility(entity, other);
        ...
        if (potentialPartners.length > 0) {
            ...
            // 関係形成の確率（相性に依存）
            const formationChance = potentialPartners[0].compatibility * 0.2;
