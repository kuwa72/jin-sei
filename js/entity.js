...
class Entity {
    ...
    // 色の計算
    calculateColor() {
        const genderBase = this.gender === 0 ? 
            {r: 255, g: 150, b: 150} : // 女性: ピンク系
    ...
    interact() {
        ...
        if (this.partner && this.gender === 0) { // 女性のみが子を産む
    ...
    checkNewRelationships() {
        ...
        for (const other of entities) {
            ...
            if (other === this || other.isDead || other.lifeStage !== 1 || other.gender === this.gender || other.partner) {
                continue;
            }
            ...
            if (distance < 50) { // 近くにいる場合
                ...
                // 相性が良ければ関係を形成
                if (compatibilityScore > 0.7 && Math.random() < 0.1) {
    ...
    calculateCompatibility(other) {
        // 外向性の補完性 (差が小さいほど良い)
        const extroversionMatch = 1 - Math.abs(this.extroversion - other.extroversion);
    ...
    addRomanticInterest(other) {
        ...
        if (!this.romanticInterests.includes(other)) {
            ...
            }, 5000 + Math.random() * 5000); // 5-10秒後に解消
        }
    }
    ...
    checkReproduction() {
        // 繁殖確率 (年齢と関係の長さに依存)
        const reproductionChance = 0.0004;
    ...
    inheritTraits(child) {
        ...
        child.extroversion = (this.extroversion + this.partner.extroversion) / 2 + (Math.random() - 0.5) * 0.2;
        child.agreeableness = (this.agreeableness + this.partner.agreeableness) / 2 + (Math.random() - 0.5) * 0.2;
